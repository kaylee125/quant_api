...
async def get_price(
    ticker: str,
    start_date: date = settings.DEFAULT_FROM_DATE,
    end_date: date = settings.DEFAULT_TO_DATE
):
    '''
    ...
    '''
    df = get_ohlc(ticker, start_date=start_date, end_date=end_date, decimal_duex=True)
    df.rename(columns={'Adj Close':ticker}, inplace=True)
    # return df[[ticker]]
    return df[[ticker]].reset_index().to_dict(orient='records')
